...
if (isset($_GET['action'])) {
    if ($_GET['action'] == "hapus") {
        $id = $_GET['id'];
        $sql = mysqli_query($kws, "DELETE FROM tbl_ortu WHERE id = '$id'");
        if ($sql) {
...
<section id="tampilMahasiswa">
    <div class="header">
        <h2 class="fw-bolder">Data Orang Tua</h2>
        <a href="index.php?page=_admOrangTua" class="btn btn-primary"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus-square-fill me-3 mb-1" viewBox="0 0 16 16">
    ...
    <div class="table-data mt-3">
        <table id="myTable" class="display table table-striped table-responsive table-hover">
            ...
            <tbody>
            <?php
            $query = mysqli_query($kws, "SELECT * FROM tbl_ortu");
            while($result = mysqli_fetch_assoc($query)){
                ?>
                <tr>
                    <td><?php echo $result['id']?></td>
                    <td><?php echo $result['ayah']?></td>
                    <td><?php echo $result['ibu']?></td>
                    <td><?php echo $result['alamat']?></td>
                    <td class="w-25"><img class="img-thumbnail w-10" src="file/<?php echo $result['foto']?>" alt=""></td>
                    <td>
                        <a href="index.php?page=_editOrangTua&id=<?php echo $result['id']?>" class="btn btn-success">
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a id="liveToastBtn" class="btn btn-danger" href="index.php?page=_tampDataOrangTua&action=hapus&id=<?php echo $result['id']; ?>">Hapus</a>
                                    </div>
                                </div>
                            </div>
                        </div>
